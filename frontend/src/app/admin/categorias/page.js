...
export default function CategoriasPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      // Aseguramos que cargamos el array de datos
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">Organiza tus documentos técnicos</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <button 
            ...
            className="mt-4 flex items-center text-blue-600 hover:text-blue-700 font-semibold"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {categories.length > 0 ? (
            categories.map((category) => (
              <div key={category.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between hover:shadow-md transition-shadow group relative overflow-hidden">
                <div className="absolute top-0 right-0 p-1 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-50 rounded-bl-xl border-l border-b border-gray-100 flex">
                   <button 
                ...
                <div className="mt-6 flex items-center text-sm text-gray-500 bg-gray-50 px-3 py-2 rounded-lg">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-lg font-bold text-gray-800">{currentCategory ? 'Editar Categoría' : 'Nueva Categoría'}</h3>
