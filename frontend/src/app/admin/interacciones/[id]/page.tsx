...
import {
  ...
  ThumbsUp, ThumbsDown, Minus, AlertCircle, Loader2,
  MessageSquare, Bot, Hash, Coins,
} from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'

import { getInteraction, type InteractionDetail } from '@/lib/api/interactions'

import { Button } from '@/components/ui/button'

// ── FeedbackBadge ─────────────────────────────────────────────────────────────

function FeedbackBadge({ value }: { value: InteractionDetail['feedback'] }) {
  if (value === 'positive') {
    return (
...
export default function InteraccionDetailPage() {
  ...
  const [item, setItem]         = useState<InteractionDetail | null>(null)
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (result.success) {
        ...
        if (result.error?.includes('404') || result.error?.includes('no encontrad')) {
          setNotFound(true)
        } else {
          setError(result.error || 'No se pudo cargar la interacción.')
        }
      }
      setLoading(false)
    })()
    ...
    return (
      <div className="space-y-4">
        <Button
          ...
          className="gap-2 text-sm border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400"
        >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-gray-100 dark:border-gray-700">
              ...
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Pregunta del técnico</h3>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-gray-100 dark:border-gray-700">
              ...
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Respuesta IA</h3>
            </div>
          ...
          <div className="px-5 py-1">
            ...
            <MetaRow icon={Clock} label="Tiempo de respuesta">
              {item.responseTimeMs != null
                ? <span className="font-mono">{item.responseTimeMs} ms</span>
                : '—'}
            </MetaRow>
            ...
            <MetaRow icon={Hash} label="Tokens">
              {(item.tokensInput != null || item.tokensOutput != null)
                ? (
                  <span className="font-mono text-xs">
                    {item.tokensInput != null ? `↑ ${item.tokensInput}` : '—'}
                    {' / '}
                    {item.tokensOutput != null ? `↓ ${item.tokensOutput}` : '—'}
                  </span>
                )
                : '—'}
            </MetaRow>

            <MetaRow icon={Coins} label="Coste estimado">
              {'—'}
            </MetaRow>
