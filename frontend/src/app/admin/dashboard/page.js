  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Métricas en tiempo real</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Top 5 preguntas */}
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
          <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4">Top 5 preguntas frecuentes</h3>
          <ol className="space-y-3">
            {stats.top_questions.map((q, idx) => (
              <li key={idx} className="flex items-center gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 text-xs font-bold flex items-center justify-center">
                  {idx + 1}
                </span>
                <span className="text-sm text-gray-700 dark:text-gray-300 flex-1 truncate" title={q.query}>{q.query}</span>
                <span className="flex-shrink-0 text-xs font-semibold text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700 px-2 py-0.5 rounded-full">
                  ×{q.total}
                </span>
              </li>
            ))}
            {stats.top_questions.length === 0 && (
              <p className="text-sm text-gray-400 dark:text-gray-500">Sin datos aún</p>
            )}
          </ol>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
          <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4">Feedback de respuestas</h3>
          {stats.feedback.total > 0 ? (
            <>
              <div className="flex gap-2 h-4 rounded-full overflow-hidden mb-3">
                <div className="bg-green-400 transition-all" style={{ width: `${stats.feedback.positive_pct}%` }} />
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500">Sin valoraciones aún</p>
          )}
        </div>

        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
          <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4">Caché semántica</h3>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.cache.entries}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Entradas</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.cache.hits}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Hits totales</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.cache.hit_rate}%</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Hit rate</p>
            </div>
          </div>
