    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Consulta tus conversaciones anteriores</p>
        </div>
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-white border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm transition-all"
          />
        </div>
      </div>

      {loading && (
        <div className="py-20 text-center text-gray-400">Cargando historial...</div>
      )}
        ...
        <div className="space-y-8">
          {groups.map(([date, items]) => (
            <div key={date} className="space-y-3">
              <div className="flex items-center text-sm font-semibold text-gray-400 uppercase tracking-wider">
              ...
              <div className="grid gap-3">
                {items.map((item) => (
                  <div
                    ...
                    className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between hover:shadow-md hover:border-blue-100 transition-all cursor-pointer group"
                  >
                    <div className="flex items-center space-x-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-bold text-gray-900 truncate">{item.query}</h4>
                        <div className="flex items-center mt-1 space-x-3 text-xs text-gray-500">
                          <span>{new Date(item.created_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</span>
                          <span>•</span>
                          <span className="truncate max-w-xs text-gray-400">{item.response.slice(0, 80)}{item.response.length > 80 ? '…' : ''}</span>
                        </div>
                      </div>
                    </div>
